...
- **Scopes gate your automation; roles gate your teammates.** A role gap hides UI controls rather than explaining itself, so it reads as a broken board (the same call over the API does return a 403). Triage is the minimum for labels/assignees/sub-issues, project Write for Status/Priority. Team grants need `admin:org`, which a default `gh` login does not carry (an org-settings PATCH also accepts `repo`). See `references/access-and-roles.md`.
