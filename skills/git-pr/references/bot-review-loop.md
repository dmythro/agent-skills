...
#       "⚠️ Action not completed / Review rate limited." -> bounced, with a "Next review available
...
# "review rate limited" matches the bounced-trigger ack, which carries the same "Next review
# available in: N minutes" window -- bot_status dates and honors it like any limit notice.
BOT_LIMIT_RE='review limit reached|rate limit exceeded|review rate limited'
...
The PR-side quota is **per developer, not per PR** (docs: limits are enforced per developer over rolling hourly windows): one bucket spans every PR in every repo, so all of a developer's open PRs contend for the same review windows -- including PRs being merged in parallel, whose auto-reviews consume slots just like re-requests. On lower plans and trials the effective rate is roughly **one review per ~25-30 minutes**, so several PRs in flight is a scheduling problem, not N independent loops:

1. **Drafts are the queue control.** Draft PRs are excluded from auto-review and cost nothing; **marking one ready IS the review request** -- promote one PR at a time, when it is that PR's turn for the window.
2. **One window, one PR, one trigger.** Give the next window to the PR closest to merge. Parallel triggers race for a single slot: the losers bounce with the rate-limit ack, and a bounced request is never queued (Per-Bot Setup) -- it must be re-posted in a later window.
