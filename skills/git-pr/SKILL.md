...
| PR exists, a bot in `reviewRequests` (e.g. `copilot-pull-request-reviewer`) | Auto-review in flight -- wait for it (Bot Review Loop); do NOT re-request |
...
**Auto-review is common (GitHub):** many repos/orgs enable automatic Copilot code review (repo/org settings or rulesets), which requests Copilot on every **non-draft** PR at creation (and when a draft is marked ready for review). So a freshly created PR often already has a pending bot request or bot comments minutes later -- check before requesting a review or you get a duplicate round.
...
After creating a **non-draft** GitHub PR, check `gh pr view --json reviewRequests` before requesting any bot review -- repos with automatic Copilot review already have one in flight.
