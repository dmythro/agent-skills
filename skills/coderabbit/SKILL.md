...
**The table is a CEILING, not the rate you get -- fair-usage throttling sets the effective PR-side allowance from your trailing-7-day usage.** Each review's own run-configuration block states it outright ("Your included PR review attempts over the past 7 days set your current allowance at N reviews per hour"). Documented Pro tiers by trailing-7-day PR review count: 30--39 -> 4/hr, 40--49 -> 3/hr, 50--59 -> 2/hr, 60+ -> 1/hr -- observed live crossing a tier mid-loop (4/hr at round 1, 3/hr two rounds later, then a bounce, on a paid Pro plan). Two consequences: **a heavy review week throttles next week too** (the window is 7 days, not the hour), and **bounced attempts count as attempts** -- a retry that bounces both fails and shrinks the future budget, so never hammer a closed window. The [usage-based add-on](https://docs.coderabbit.ai/management/usage-based-addon) is the release valve: it bills past the throttle instead of bouncing.

**Which plan row applies is something you must be told -- but the LIVE allowance is reported in one place:** the "Run configuration" block inside each posted review names the plan and the current reviews-per-hour figure, so read it off the most recent review before planning a loop. Everything else is silent: `coderabbit usage` reports the billing period (org, overage on/off, review count, reset date) and the PR-side API never mentions a plan. Declare it in the **Code Review Policy** (`git-pr` skill) -- `CodeRabbit plan: pro+ until 2026-08-20, then pro` -- in the repo's AGENTS.md/CLAUDE.md, or globally for every repo. It decides real behaviour: how many PRs can be non-draft at once, how many rounds to budget, and how much iteration belongs in the CLI lane instead. Re-plan when a trial lapses; a queue tuned for 10 reviews/hour stalls at 5. Undeclared, take the floor from observed behaviour (`bot_bucket`) rather than assuming the best case.
